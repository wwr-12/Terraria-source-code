...
public class NetClientSocialModule : NetSocialModule
{
	...
	public override void LaunchLocalServer(Process process, ServerMode mode)
	{
		...
		rail_api.RailFactory().RailUtils().GetLaunchAppParameters((EnumRailLaunchAppType)2, out text);
	...
	public unsafe void OnRailEvent(RAILEventID id, EventBase data)
	{
		...
		WeGameHelper.WriteDebugString("OnRailEvent,id=" + ((object)(*(RAILEventID*)(&id))/*cast due to constrained. prefix*/).ToString() + " ,result=" + data.result.ToString());
		if ((int)id <= 12010)
		{
			if ((int)id != 12002)
			{
