...
public class UILinkPointNavigator
{
	...
	{
		UILinkPoint value;
		Vector2 vector = (Points.TryGetValue(ID, out value) ? value.Position : Vector2.Zero);
		if (vector == Vector2.Zero)
		{
			if (ID >= 180 && ID <= 184)
			{
				vector = GetPosition(ID - 180 + 100);
			}
			else if (ID >= 185 && ID <= 189)
			{
				vector = GetPosition(ID - 185 + 110);
			}
		}
	...
	private static void DrawLink(UILinkPoint src, int targetId, Vector2 dir, Color colorStart, Color colorEnd)
	{
		if (Points.TryGetValue(targetId, out var value) && !(value.Position == Vector2.Zero))
		{
			...
			Vector2 vector = dir.RotatedBy(Math.PI / 2.0);
			Vector2 vector2 = src.Position / Main.UIScale + vector * 2f;
			Vector2 vector3 = value.Position / Main.UIScale + vector * 2f;
			if (Vector2.Dot(vector3 - vector2, dir) < 0f)
			{
				DebugLineDraw.UI.AddLine(vector2, vector2 += (vector + dir * 2f) * 2f * 2f, colorStart);
				DebugLineDraw.UI.AddLine(vector3, vector3 += (vector - dir * 2f) * 2f * 2f, colorEnd);
			}
			DebugLineDraw.UI.AddLine(vector2, vector3, colorStart, colorEnd);
		}
	}
}
