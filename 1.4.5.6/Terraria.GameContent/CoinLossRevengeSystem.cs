...
public class CoinLossRevengeSystem
{
	...
	{
		if (npc.boss || (npc.realLife != -1 && npc.realLife != npc.whoAmI) || npc.rarity > 0 || npc.extraValue < MinimumCoinsForCaching || npc.position.X < Main.leftWorld + 640f + 16f || npc.position.X + (float)npc.width > Main.rightWorld - 640f - 32f || npc.position.Y < Main.topWorld + 640f + 16f || npc.position.Y > Main.bottomWorld - 640f - 32f - (float)npc.height)
		{
			return;
		}
		int num = npc.netID;
