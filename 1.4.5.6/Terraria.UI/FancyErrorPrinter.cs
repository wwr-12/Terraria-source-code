...
public class FancyErrorPrinter
{
	public static void ShowFailedToLoadAssetError(Exception exception, string filePath)
	{
		...
		if (flag)
		{
			...
			List<string> suggestions = new List<string> { "Try to verify/repair the game installation, the asset may be missing.", "If you are using an Anti-virus, please make sure it does not block Terraria in any way." };
	...
	public static void ShowFileSavingFailError(Exception exception, string filePath)
	{
		...
		if (flag)
		{
			...
			List<string> list = new List<string> { "If you are using an Anti-virus, please make sure it does not block Terraria in any way.", "Try making sure your `Documents/My Games/Terraria` folder is not set to 'read-only'.", "Try to verify/repair the game installation." };
