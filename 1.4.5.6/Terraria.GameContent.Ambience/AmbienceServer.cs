...
public class AmbienceServer
{
	...
	public void Update()
	{
		...
		IEnumerable<SkyEntityType> source = from pair in _spawnConditions
			where pair.Value()
			select pair.Key;
		if (source.Count((SkyEntityType skyEntityType) => true) == 0)
		{
			return;
		}
		...
		IEnumerable<SkyEntityType> source2 = source.Where((SkyEntityType skyEntityType) => IsPlayerAtRightHeightForType(skyEntityType, player) && _secondarySpawnConditionsPerPlayer.ContainsKey(skyEntityType) && _secondarySpawnConditionsPerPlayer[skyEntityType](player));
		int num = source2.Count((SkyEntityType skyEntityType) => true);
		if (num == 0 || Main.rand.Next(5) < 3)
		{
			source2 = source.Where((SkyEntityType skyEntityType) => IsPlayerAtRightHeightForType(skyEntityType, player) && (!_secondarySpawnConditionsPerPlayer.ContainsKey(skyEntityType) || _secondarySpawnConditionsPerPlayer[skyEntityType](player)));
			num = source2.Count((SkyEntityType skyEntityType) => true);
		}
		if (num != 0)
		{
			SkyEntityType type = source2.ElementAt(Main.rand.Next(num));
			SpawnForPlayer(player, type);
		}
	}
