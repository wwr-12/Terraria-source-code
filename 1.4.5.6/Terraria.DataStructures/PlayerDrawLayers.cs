...
public static class PlayerDrawLayers
{
	...
	public static void DrawPlayer_09_Wings(ref PlayerDrawSet drawinfo)
	{
		...
		else if (drawinfo.drawPlayer.wings == 31)
		{
			...
			color12 = Color.Lerp(Color.HotPink, Color.Crimson, (float)Math.Cos((float)Math.PI * 2f * ((float)drawinfo.drawPlayer.miscCounter / 100f)) * 0.4f + 0.5f);
			color12.A = 0;
			for (int n = 0; n < 4; n++)
			{
	...
	public static void DrawPlayer_27_HeldItem(ref PlayerDrawSet drawinfo)
	{
		...
		case 3476:
		{
			...
			Rectangle rectangle2 = value2.Frame(1, 9, 0, drawinfo.drawPlayer.miscCounter % 54 / 6);
			Vector2 vector4 = new Vector2(rectangle2.Width / 2 * drawinfo.drawPlayer.direction, 0f);
			Vector2 origin2 = rectangle2.Size() / 2f;
			item = new DrawData(value2, (drawinfo.ItemLocation - Main.screenPosition + vector4).Floor(), rectangle2, heldItem.GetAlpha(drawinfo.itemColor).MultiplyRGBA(new Color(new Vector4(0.5f, 0.5f, 0.5f, 0.8f))), drawinfo.drawPlayer.itemRotation, origin2, adjustedItemScale, drawinfo.itemEffect);
			drawinfo.DrawDataCache.Add(item);
			value2 = TextureAssets.GlowMask[195].Value;
			item = new DrawData(value2, (drawinfo.ItemLocation - Main.screenPosition + vector4).Floor(), rectangle2, new Color(250, 250, 250, heldItem.alpha) * 0.5f, drawinfo.drawPlayer.itemRotation, origin2, adjustedItemScale, drawinfo.itemEffect);
			drawinfo.DrawDataCache.Add(item);
			return;
		}
		case 4049:
		{
			...
			Rectangle rectangle3 = value3.Frame(1, 4, 0, drawinfo.drawPlayer.miscCounter % 20 / 5);
			...
			item = new DrawData(value3, (drawinfo.ItemLocation - Main.screenPosition + vector5).Floor(), rectangle3, heldItem.GetAlpha(drawinfo.itemColor), drawinfo.drawPlayer.itemRotation, origin3, adjustedItemScale, drawinfo.itemEffect);
			drawinfo.DrawDataCache.Add(item);
			return;
		}
		case 3779:
		...
		if (heldItem.type == 5462 && drawinfo.SelectedDrawnProjectile != null)
		{
			...
			if (selectedDrawnProjectile2.active && selectedDrawnProjectile2.type == 1040)
			{
				...
				num18 *= 0.85f;
				item = new DrawData(value4, position2, null, color * num12, 0f, value4.Frame().Size() / 2f, num18, drawinfo.itemEffect);
				drawinfo.DrawDataCache.Add(item);
				item = new DrawData(value4, position2, null, Color.White * num12, 0f, value4.Frame().Size() / 2f, num18 * 0.92f, drawinfo.itemEffect);
				drawinfo.DrawDataCache.Add(item);
			}
		}
