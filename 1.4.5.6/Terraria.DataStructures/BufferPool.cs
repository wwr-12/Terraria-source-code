...
public static class BufferPool
{
	...
	public static CachedBuffer Request(int size)
	{
		lock (bufferLock)
		{
			...
			if (size <= 65536)
			{
				if (HugeBufferQueue.Count == 0)
				{
					return new CachedBuffer(new byte[65536]);
				}
				return HugeBufferQueue.Dequeue().Activate();
			}
	...
	public static void Recycle(CachedBuffer buffer)
	{
		...
		lock (bufferLock)
		{
			...
			{
				HugeBufferQueue.Enqueue(buffer);
			}
		}
	}
	...
	public static void PrintBufferSizes()
	{
		lock (bufferLock)
		{
			...
			Console.WriteLine("HugeBufferQueue.Count: " + HugeBufferQueue.Count);
			Console.WriteLine("");
		}
	}
}
