...
public static class LaunchInitializer
{
	...
	private static void LoadServerParameters(Main game)
	{
		...
		string hostToken;
		if ((hostToken = TryParameter("-hosttoken")) != null)
		{
		...
		string text;
		if ((text = TryParameter("-world")) != null)
		{
			if (FileUtilities.Exists(text, cloud: false) || !Main.autoGen)
			{
				game.SetWorld(text, cloud: false);
			}
			else
			{
				new WorldFileData(text, cloudSave: false).SetAsActive();
				Main.autoGenFileLocation = text;
			}
		}
		else if (SocialAPI.Mode == SocialMode.Steam && (text = TryParameter("-cloudworld")) != null)
		{
			if (FileUtilities.Exists(text, cloud: true) || !Main.autoGen)
			{
				game.SetWorld(text, cloud: true);
			}
			else
			{
				new WorldFileData(text, cloudSave: true).SetAsActive();
				Main.autoGenFileLocation = text;
			}
		}
		string configPath;
