...
namespace Terraria
{
	public class Collision
	{
		...
		public static bool DrownCollision(Vector2 Position, int Width, int Height, float gravDir = -1f)
		{
			...
			for (int i = num3; i < value2; i++)
			{
				for (int j = value3; j < value4; j++)
				{
					...
					if (tile != null && tile.liquid > 0 && !tile.lava() && (j != num4 || !tile.active() || !Main.tileSolid[tile.type] || Main.tileSolidTop[tile.type]))
					{
						vector2.X = i * 16;
						vector2.Y = j * 16;
						int num5 = 16;
			...
			for (int i = num; i < value2; i++)
			{
				for (int j = value3; j < value4; j++)
				{
					...
					if (Main.tile[i, j].slope() > 0)
					{
						...
						{
							if (Main.tile[i, j].slope() == 1 && vector.Y + (float)Height - Math.Abs(Velocity.X) - 1f <= vector4.Y + (float)num2 && vector.X >= vector4.X)
							{
								flag = true;
							}
							if (Main.tile[i, j].slope() == 2 && vector.Y + (float)Height - Math.Abs(Velocity.X) - 1f <= vector4.Y + (float)num2 && vector.X + (float)Width <= vector4.X + 16f)
							{
								flag = true;
							}
						}
				...
				for (int j = num3; j < num4; j++)
				{
					...
					if (type == 32 || type == 69 || type == 80 || type == 352 || (type == 80 && Main.expertMode))
					{
						...
						if (type == 32 || type == 69 || type == 352)
						{
							...
							{
								NetMessage.SendData(17, -1, -1, null, 4, i, j);
							}
						}
		...
		public static void StepConveyorBelt(Entity entity, float gravDir)
		{
			...
			List<Point> entityEdgeTiles = GetEntityEdgeTiles(entity, left: false, right: false);
			...
			foreach (Point item in entityEdgeTiles)
			{
				...
				int num3 = TileID.Sets.ConveyorDirection[tile.type];
				if (num3 == 0)
				{
					continue;
				}
				lineStart.X = (lineStart2.X = item.X * 16);
				lineEnd.X = (lineEnd2.X = item.X * 16 + 16);
				switch (tile.slope())
				{
				case 1:
					lineStart2.Y = item.Y * 16;
					lineEnd2.Y = (lineEnd.Y = (lineStart.Y = item.Y * 16 + 16));
					break;
				case 2:
					lineEnd2.Y = item.Y * 16;
					lineStart2.Y = (lineEnd.Y = (lineStart.Y = item.Y * 16 + 16));
					break;
				case 3:
					lineEnd.Y = (lineStart2.Y = (lineEnd2.Y = item.Y * 16));
					lineStart.Y = item.Y * 16 + 16;
					break;
				case 4:
					lineStart.Y = (lineStart2.Y = (lineEnd2.Y = item.Y * 16));
					lineEnd.Y = item.Y * 16 + 16;
					break;
				default:
					if (tile.halfBrick())
					{
						lineStart2.Y = (lineEnd2.Y = item.Y * 16 + 8);
					}
					else
					{
						lineStart2.Y = (lineEnd2.Y = item.Y * 16);
					}
					lineStart.Y = (lineEnd.Y = item.Y * 16 + 16);
					break;
				}
				int num4 = 0;
