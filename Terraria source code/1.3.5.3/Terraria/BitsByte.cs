...
namespace Terraria
{
	public struct BitsByte
	{
		...
		public static void SortOfAUnitTest()
		{
			...
			array[3] = true;
			array[14] = true;
			BitsByte[] array2 = ComposeBitsBytesChain(optimizeLength: false, array);
			...
			BitsByte[] array4 = DecomposeBitsBytesChain(reader);
			string text = "";
			string text2 = "";
			array3 = array2;
			foreach (BitsByte bitsByte in array3)
			{
				text = text + (byte)bitsByte + ", ";
			}
			array3 = array4;
			foreach (BitsByte bitsByte2 in array3)
			{
				text2 = text2 + (byte)bitsByte2 + ", ";
			}
			Main.NewText("done");
		}
	}
