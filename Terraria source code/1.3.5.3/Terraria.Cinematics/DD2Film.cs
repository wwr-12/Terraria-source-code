...
namespace Terraria.Cinematics
{
	public class DD2Film : Film
	{
		...
		{
			int num = NPC.NewNPC((int)_portal.Center.X, (int)_portal.Bottom.Y, 568);
		...
		{
			int num = NPC.NewNPC((int)_portal.Center.X, (int)_portal.Bottom.Y, 561);
			NPC nPC = Main.npc[num];
			nPC.knockBackResist = 0f;
			nPC.immortal = true;
			nPC.dontTakeDamage = true;
		...
		{
			int num = NPC.NewNPC((int)_portal.Center.X, (int)_portal.Bottom.Y, 552);
			NPC nPC = Main.npc[num];
			nPC.knockBackResist = 0f;
			nPC.immortal = true;
			nPC.dontTakeDamage = true;
		...
		private void CreateCritters(FrameEventData evt)
		{
			...
			if (_dryad != null)
			{
				...
				{
					_ = (float)j / 10f;
					int num2 = NPC.NewNPC((int)_dryad.position.X + Main.rand.Next(-1000, 800), (int)_dryad.position.Y - Main.rand.Next(-50, 300), 356);
		...
		private void DryadPortalKnock(FrameEventData evt)
		{
			if (_dryad != null)
			{
				if (evt.Frame == 20)
				{
					...
					Main.PlaySound(3, (int)_dryad.Center.X, (int)_dryad.Center.Y);
				}
				if (evt.Frame >= 20)
				{
					_dryad.ai[0] = 1f;
		...
		{
			int num = NPC.NewNPC((int)_portal.Center.X, (int)_portal.Bottom.Y, 576);
			_ogre = Main.npc[num];
		...
		private NPC PlaceNPCOnGround(int type, Vector2 position)
		{
			...
			int num3 = NPC.NewNPC(num, num2, type, start);
