	...
	public class PlayerDeathReason
	{
		...
		public static PlayerDeathReason ByProjectile(int playerIndex, int projectileIndex)
		{
			...
			if (playerIndex >= 0 && playerIndex <= 255)
			{
				...
				playerDeathReason.SourceItemPrefix = Main.player[playerIndex].inventory[Main.player[playerIndex].selectedItem].prefix;
			}
		...
		public NetworkText GetDeathText(string deadPlayerName)
		{
			...
			return Lang.CreateDeathMessage(deadPlayerName, SourcePlayerIndex, SourceNPCIndex, SourceProjectileIndex, SourceOtherIndex, SourceProjectileType, SourceItemType);
		}
