	...
	public class NebulaSky : CustomSky
	{
		...
		public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
		{
			if (maxDepth >= float.MaxValue && minDepth < float.MaxValue)
			{
				...
				spriteBatch.Draw(_planetTexture, vector + new Vector2(-200f, -200f) + vector2, null, Color.White * 0.9f * _fadeOpacity, 0f, new Vector2(_planetTexture.Width >> 1, _planetTexture.Height >> 1), 1f, SpriteEffects.None, 1f);
			}
			int num = -1;
			int num2 = 0;
			...
			for (int j = num; j < num2; j++)
			{
				...
				if (rectangle.Contains((int)position.X, (int)position.Y))
				{
					...
					spriteBatch.Draw(_beamTexture, position, null, Color.White * 0.2f * num3 * _fadeOpacity, 0f, Vector2.Zero, new Vector2(num4 / 70f, num4 / 45f), SpriteEffects.None, 0f);
					int num5 = 0;
					for (float num6 = 0f; num6 <= 1f; num6 += 0.03f)
					{
						float num7 = 1f - (num6 + Main.GlobalTime * 0.02f + (float)Math.Sin((float)j)) % 1f;
						spriteBatch.Draw(_rockTextures[num5], position + new Vector2((float)Math.Sin(num6 * 1582f) * (num4 * 0.5f) + num4 * 0.5f, num7 * 2000f), null, Color.White * num7 * num3 * _fadeOpacity, num7 * 20f, new Vector2(_rockTextures[num5].Width >> 1, _rockTextures[num5].Height >> 1), 0.9f, SpriteEffects.None, 0f);
