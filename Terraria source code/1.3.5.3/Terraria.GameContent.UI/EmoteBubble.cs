	...
	public class EmoteBubble
	{
		...
		private void Draw(SpriteBatch sb)
		{
			...
			if (emote >= 0)
			{
				...
				sb.Draw(texture2D, vector, texture2D.Frame(8, 35, emote * 2 % 8 + frame, 1 + emote / 4), Color.White, 0f, origin, 1f, effect, 0f);
			}
			else if (emote == -1)
			{
		...
		private void ProbeEmotions(List<int> list)
		{
			...
			{
				int item = Utils.SelectRandom<int>(Main.rand, 16, 1);
				list.Add(item);
				list.Add(item);
				list.Add(item);
			}
