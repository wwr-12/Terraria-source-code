...
namespace Terraria.Graphics.Capture
{
	internal class CaptureCamera
	{
		...
		public CaptureCamera(GraphicsDevice graphics)
		{
			...
			try
			{
				_frameBuffer = new RenderTarget2D(graphics, 2048, 2048, mipMap: false, graphics.PresentationParameters.BackBufferFormat, DepthFormat.Depth24);
			}
			catch
			{
		...
		public void Capture(CaptureSettings settings)
		{
			...
			if (settings.UseScaling)
			{
				...
				_scaledFrameBuffer = new RenderTarget2D(_graphics, num2, num2, mipMap: false, _graphics.PresentationParameters.BackBufferFormat, DepthFormat.Depth24);
			}
			else
			{
