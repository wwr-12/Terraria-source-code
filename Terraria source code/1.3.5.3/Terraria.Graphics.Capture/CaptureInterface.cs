	...
	public class CaptureInterface
	{
		...
		private class ModeEdgeSelection : CaptureInterfaceMode
		{
			...
			public override void Draw(SpriteBatch sb)
			{
				...
				{
					sb.End();
					sb.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.None, RasterizerState.CullCounterClockwise, null, Main.CurrentWantedZoomMatrix);
					PlayerInput.SetZoom_Context();
					DrawMarkedArea(sb);
					DrawCursors(sb);
					sb.End();
		...
		private static bool GetMapCoords(int PinX, int PinY, int Goal, out Point result)
		{
			...
			float num11 = num;
			float num12 = num2;
			float num13 = Main.mapTexture.Width;
			float num14 = Main.mapTexture.Height;
		...
		private void DrawCameraLock(SpriteBatch sb)
		{
			...
			if (!(num <= 0f))
			{
				...
				ChatManager.DrawColorCodedStringWithShadow(sb, Main.fontDeathText, text, new Vector2(Main.screenWidth, Main.screenHeight) / 2f + vector3, Color.White * num, 0f, Vector2.Zero, Vector2.One);
				ChatManager.DrawColorCodedStringWithShadow(sb, Main.fontDeathText, text2, new Vector2(Main.screenWidth, Main.screenHeight) / 2f + vector4, Color.White * num, 0f, Vector2.Zero, Vector2.One);
			}
		}
