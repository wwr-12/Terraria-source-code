...
namespace Terraria.UI.Gamepad
{
	public class UILinkPointNavigator
	{
		...
		public static void Update()
		{
			...
			bool num = PlayerInput.Triggers.Current.HotbarPlus && !PlayerInput.Triggers.Current.HotbarMinus;
			if (!flag3)
			{
				PageLeftCD = 0;
			}
			...
			flag3 = flag3 && PageLeftCD == 0;
			bool num2 = num && PageRightCD == 0;
			...
			int num3 = 10;
			if (!Main.gameMenu && Main.playerInventory && !Main.ingameOptionsWindow && !Main.inFancyUI && (CurrentPage == 0 || CurrentPage == 4 || CurrentPage == 2 || CurrentPage == 1))
			{
		...
		{
			string text = Pages[CurrentPage].SpecialInteractions();
			string text2 = Points[CurrentPoint].SpecialInteractions();
