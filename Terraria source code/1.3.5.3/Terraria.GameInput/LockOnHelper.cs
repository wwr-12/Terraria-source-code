...
namespace Terraria.GameInput
{
	public class LockOnHelper
	{
		...
		{
			_threeDSTarget = -1;
			if (UseMode == LockOnMode.ThreeDS && PlayerInput.UsingGamepad)
			{
		...
		private static void RefreshTargets(Vector2 position, float radius)
		{
			...
			{
				NPC nPC = Main.npc[i];
				if (ValidTarget(nPC) && !(nPC.Distance(position) > radius) && rectangle.Intersects(nPC.Hitbox) && !(Lighting.GetSubLight(nPC.Center).Length() / 3f < 0.01f) && (UseMode != LockOnMode.ThreeDS || !(Vector2.Dot(nPC.DirectionFrom(center), value) < 0.65f)))
				{
					_targets.Add(i);
				}
			}
		...
		{
			if (n == null || !n.active || n.dontTakeDamage || n.friendly || n.townNPC || n.life < 1 || n.immortal)
			{
				return false;
			}
		...
		public static void Draw(SpriteBatch spriteBatch)
		{
			...
			t2.A = 220;
			float num = 0.94f + (float)Math.Sin(Main.GlobalTime * ((float)Math.PI * 2f)) * 0.06f;
			t2 *= num;
			t *= num;
			...
			for (int i = 0; i < _drawProgress.GetLength(0); i++)
			{
				...
				else if ((flag && _targets.Contains(i)) || (UseMode == LockOnMode.ThreeDS && _threeDSTarget == i))
				{
					num8 = 1;
				}
				...
				if (num9 > 0f)
				{
					...
					Vector2 pos = Main.npc[i].Top + new Vector2(0f, 0f - num7 - num10 * num6) - Main.screenPosition;
					pos = Main.ReverseGravitySupport(pos, Main.npc[i].height);
					spriteBatch.Draw(lockOnCursorTexture, pos, rectangle, color * num9, 0f, rectangle.Size() / 2f, new Vector2(0.58f, 1f) * num2 * num4 * (1f + num9) / 2f, SpriteEffects.None, 0f);
					spriteBatch.Draw(lockOnCursorTexture, pos, rectangle2, color2 * num9 * num9, 0f, rectangle2.Size() / 2f, new Vector2(0.58f, 1f) * num2 * num4 * (1f + num9) / 2f, SpriteEffects.None, 0f);
				}
				...
				if (num11 > 0f)
				{
					...
					{
						float num14 = (float)Math.PI * 2f / num13 * (float)j + Main.GlobalTime * ((float)Math.PI * 2f) * 0.25f;
						...
						spriteBatch.Draw(lockOnCursorTexture, pos3, rectangle, t * num11, rotation, rectangle.Size() / 2f, new Vector2(0.58f, 1f) * num2 * num5 * (1f + num11) / 2f, SpriteEffects.None, 0f);
						spriteBatch.Draw(lockOnCursorTexture, pos3, rectangle2, t2 * num11 * num11, rotation, rectangle2.Size() / 2f, new Vector2(0.58f, 1f) * num2 * num5 * (1f + num11) / 2f, SpriteEffects.None, 0f);
					}
				}
			}
		}
