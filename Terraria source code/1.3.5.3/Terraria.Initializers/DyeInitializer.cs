...
namespace Terraria.Initializers
{
	public static class DyeInitializer
	{
		...
		private static void LoadArmorDyes()
		{
			...
			GameShaders.Armor.BindShader(3039, new ArmorShaderData(pixelShaderRef, "ArmorTwilight")).UseImage("Images/Misc/noise").UseColor(0.5f, 0.1f, 1f);
		...
		private static void LoadHairDyes()
		{
			Ref<Effect> pixelShaderRef = Main.PixelShaderRef;
			LoadLegacyHairdyes();
			GameShaders.Hair.BindShader(3259, new HairShaderData(pixelShaderRef, "ArmorTwilight")).UseImage("Images/Misc/noise").UseColor(0.5f, 0.1f, 1f);
		}
		...
		private static void LoadLegacyHairdyes()
		{
			...
			GameShaders.Hair.BindShader(1983, new LegacyHairShaderData().UseLegacyMethod(delegate(Player player, Color newColor, ref bool lighting)
			{
				...
				color = ((Main.waterStyle == 2) ? new Color(124, 118, 242) : ((Main.waterStyle == 3) ? new Color(143, 215, 29) : ((Main.waterStyle == 4) ? new Color(78, 193, 227) : ((Main.waterStyle == 5) ? new Color(189, 231, 255) : ((Main.waterStyle == 6) ? new Color(230, 219, 100) : ((Main.waterStyle == 7) ? new Color(151, 107, 75) : ((Main.waterStyle == 8) ? new Color(128, 128, 128) : ((Main.waterStyle == 9) ? new Color(200, 0, 0) : ((Main.waterStyle != 10) ? new Color(28, 216, 94) : new Color(208, 80, 80))))))))));
			...
			GameShaders.Hair.BindShader(1984, new LegacyHairShaderData().UseLegacyMethod(delegate(Player player, Color newColor, ref bool lighting)
			{
				...
				if (!Main.gameMenu && !Main.gamePaused)
				{
					...
					if (Main.rand.Next(225) == 0)
					{
						...
						int num2 = Gore.NewGore(new Vector2(player.position.X + (float)Main.rand.Next(player.width), player.position.Y + (float)Main.rand.Next(8)), player.velocity, type2);
