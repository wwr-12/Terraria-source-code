...
namespace Terraria.GameContent.UI.Elements
{
	public class UIAchievementListItem : UIPanel
	{
		...
		public UIAchievementListItem(Achievement achievement, bool largeForOtherLanguages)
		{
			...
			if (_large)
			{
				_innerPanelBottomTexture = TextureManager.Load("Images/UI/Achievement_InnerPanelBottom_Large");
			}
			else
			{
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			string text = Main.fontItemStack.CreateWrappedText(_achievement.Description.Value, (num3 - 20f) * (1f / baseScale2.X), Language.ActiveCulture.CultureInfo);
			Vector2 stringSize = ChatManager.GetStringSize(Main.fontItemStack, text, baseScale2, num3);
			if (!_large)
			{
				stringSize = ChatManager.GetStringSize(Main.fontItemStack, _achievement.Description.Value, baseScale2, num3);
			}
			...
			vector3.Y += 2f;
			vector3.X += 4f;
			spriteBatch.Draw(_categoryTexture, vector3, _categoryTexture.Frame(4, 2, (int)category), base.IsMouseHovering ? Color.White : Color.Silver, 0f, Vector2.Zero, 0.5f, SpriteEffects.None, 0f);
			vector3.X += 4f;
			vector3.X += 17f;
			ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, _achievement.FriendlyName.Value, vector3, value, 0f, Vector2.Zero, baseScale, num3);
			vector3.X -= 17f;
			Vector2 position = vector2 + Vector2.UnitY * 27f + vector;
			DrawPanelBottom(spriteBatch, position, num3, color);
			position.X += 8f;
			position.Y += 4f;
			ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, text, position, value2, 0f, Vector2.Zero, baseScale2);
			if (flag)
			{
				...
				Vector2 stringSize2 = ChatManager.GetStringSize(Main.fontItemStack, text2, baseScale3);
				...
				ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, text2, vector4, value, 0f, new Vector2(0f, 0f), baseScale3, 90f);
			}
		}
