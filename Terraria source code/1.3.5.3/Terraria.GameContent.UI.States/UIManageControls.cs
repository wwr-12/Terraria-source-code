...
namespace Terraria.GameContent.UI.States
{
	public class UIManageControls : UIState
	{
		...
		{
			"Throw", "Inventory", "RadialHotbar", "RadialQuickbar", "LockOn", "sp3", "sp4", "sp5", "sp6", "sp7",
			"sp8", "sp18", "sp19", "sp9", "sp10", "sp11", "sp12", "sp13"
		};
		...
		{
			_KeyboardGamepadTexture = TextureManager.Load("Images/UI/Settings_Inputs");
			_keyboardGamepadBorderTexture = TextureManager.Load("Images/UI/Settings_Inputs_Border");
			_GameplayVsUITexture = TextureManager.Load("Images/UI/Settings_Inputs_2");
			_GameplayVsUIBorderTexture = TextureManager.Load("Images/UI/Settings_Inputs_2_Border");
			...
			_buttonProfile = new UIKeybindingSimpleListItem(() => PlayerInput.CurrentProfile.Name, new Color(73, 94, 171, 255) * 0.9f);
		...
		private void AssembleBindPanels()
		{
			List<string> bindings = new List<string>
			{
				...
				"QuickMount", "QuickHeal", "QuickMana", "QuickBuff", "Throw", "Inventory", "ViewZoomIn", "ViewZoomOut", "sp9"
			};
			List<string> bindings2 = new List<string>
			{
				"MouseLeft", "MouseRight", "Up", "Down", "Left", "Right", "Jump", "Grapple", "SmartSelect", "SmartCursor",
				"QuickMount", "QuickHeal", "QuickMana", "QuickBuff", "LockOn", "Throw", "Inventory", "sp9"
			};
		...
		public UIElement CreatePanel(string bind, InputMode currentInputMode, Color color)
		{
			...
			case "sp18":
				...
				}, delegate
				{
					...
					currentValue = UILinksInitializer.HandleSlider(currentValue, 0f, 1f, PlayerInput.CurrentProfile.InterfaceDeadzoneX);
			...
			case "sp19":
				...
				}, () => Utils.InverseLerp(4f, 12f, PlayerInput.CurrentProfile.InventoryMoveCD, clamped: true), delegate(float f)
				{
				...
				}, delegate
				{
					...
					if (UILinkPointNavigator.Shortcuts.INV_MOVE_OPTION_CD == 0)
					{
						float num = Utils.InverseLerp(4f, 12f, PlayerInput.CurrentProfile.InventoryMoveCD, clamped: true);
						float num2 = UILinksInitializer.HandleSlider(num, 0f, 1f, PlayerInput.CurrentProfile.InterfaceDeadzoneX);
						if (num != num2)
						{
							UILinkPointNavigator.Shortcuts.INV_MOVE_OPTION_CD = 8;
							int num3 = Math.Sign(num2 - num);
							PlayerInput.CurrentProfile.InventoryMoveCD = (int)MathHelper.Clamp(PlayerInput.CurrentProfile.InventoryMoveCD + num3, 4f, 12f);
						}
					}
				}, 1008, color);
			default:
