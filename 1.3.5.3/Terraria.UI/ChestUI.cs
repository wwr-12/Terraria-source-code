	...
	public class ChestUI
	{
		...
		private static void DrawName(SpriteBatch spritebatch)
		{
			...
			else if (player.chest > -1)
			{
				...
				else if (Main.tile[player.chestX, player.chestY].type == 467)
				{
			...
			Utils.WordwrapString(text, Main.fontMouseText, 200, 1, out var lineAmount);
			lineAmount++;
		...
		private static void DrawButton(SpriteBatch spriteBatch, int ID, int X, int Y)
		{
			...
			ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontMouseText, text, new Vector2(X, Y), color, 0f, vector / 2f, new Vector2(num), -1f, 1.5f);
			vector *= num;
			switch (ID)
			{
			case 0:
			...
			if (!Utils.FloatIntersect(Main.mouseX, Main.mouseY, 0f, 0f, (float)X - vector.X / 2f, (float)Y - vector.Y / 2f, vector.X, vector.Y))
			{
				UpdateHover(ID, hovering: false);
				return;
			}
		...
		private static void DrawSlots(SpriteBatch spriteBatch)
		{
			...
			for (int i = 0; i < 10; i++)
			{
				for (int j = 0; j < 4; j++)
				{
					...
					if (Utils.FloatIntersect(Main.mouseX, Main.mouseY, 0f, 0f, num, num2, (float)Main.inventoryBackTexture.Width * Main.inventoryScale, (float)Main.inventoryBackTexture.Height * Main.inventoryScale) && !PlayerInput.IgnoreMouseInterface)
					{
						player.mouseInterface = true;
		...
		public static void LootAll()
		{
			...
			if (player.chest == -4)
			{
				for (int k = 0; k < 40; k++)
				{
					if (player.bank3.item[k].type > 0)
					{
						...
						player.bank3.item[k] = player.GetItem(Main.myPlayer, player.bank3.item[k]);
					}
				}
				return;
			}
			for (int l = 0; l < 40; l++)
			{
				if (player.bank.item[l].type > 0)
				{
					...
					player.bank.item[l] = player.GetItem(Main.myPlayer, player.bank.item[l]);
				}
			}
		}
