	...
	public class DD2Event
	{
		...
		public static void CheckProgress(int slainMonsterID)
		{
			...
			if (NPC.waveKills >= (float)requiredKillCount && requiredKillCount != 0)
			{
				...
				WorldGen.BroadcastText(NetworkText.FromKey("DungeonDefenders2.WaveComplete"), INFO_NEW_WAVE_COLOR);
		...
		private static short[] GetEnemiesForWave(int wave)
		{
			...
			{
			case 3:
				return Difficulty_3_GetEnemiesForWave(wave);
			case 2:
				return Difficulty_2_GetEnemiesForWave(wave);
			default:
				return Difficulty_1_GetEnemiesForWave(wave);
			}
		}
		...
		private static int GetMonsterPointsWorth(int slainMonsterID)
		{
			...
			{
			case 3:
				return Difficulty_3_GetMonsterPointsWorth(slainMonsterID);
			case 2:
				return Difficulty_2_GetMonsterPointsWorth(slainMonsterID);
			default:
				return Difficulty_1_GetMonsterPointsWorth(slainMonsterID);
			}
		}
		...
		private static void DropStarterCrystals()
		{
			for (int i = 0; i < 200; i++)
			{
				if (Main.npc[i].active && Main.npc[i].type == 548)
				{
					for (int j = 0; j < 5; j++)
					{
						Item.NewItem(Main.npc[i].position, Main.npc[i].width, Main.npc[i].height, 3822, 2);
					}
					break;
				}
			}
		...
		private static void Difficulty_1_SpawnMonsterFromGate(Vector2 gateBottom)
		{
			...
			case 1:
				...
				{
					num4 = NPC.NewNPC(x, y, 552);
				}
				break;
			case 2:
				if (NPC.CountNPCS(552) + NPC.CountNPCS(555) < num)
				{
					num4 = ((Main.rand.Next(7) != 0) ? NPC.NewNPC(x, y, 552) : NPC.NewNPC(x, y, 555));
				}
				break;
			case 3:
				if (Main.rand.Next(6) == 0 && NPC.CountNPCS(561) < num2)
				{
					num4 = NPC.NewNPC(x, y, 561);
				}
				else if (NPC.CountNPCS(552) + NPC.CountNPCS(555) < num)
				{
					num4 = ((Main.rand.Next(5) != 0) ? NPC.NewNPC(x, y, 552) : NPC.NewNPC(x, y, 555));
				}
				break;
			case 4:
				if (Main.rand.Next(12) == 0 && NPC.CountNPCS(558) < num3)
				{
					num4 = NPC.NewNPC(x, y, 558);
				}
			...
			case 5:
			{
				...
				{
					num4 = ((Main.rand.Next(4) != 0) ? NPC.NewNPC(x, y, 552) : NPC.NewNPC(x, y, 555));
				}
				break;
			}
			default:
		...
		private static void Difficulty_2_SpawnMonsterFromGate(Vector2 gateBottom)
		{
			...
			case 3:
				...
				else if (NPC.CountNPCS(553) + NPC.CountNPCS(556) < num)
				{
					...
					{
						num7 = NPC.NewNPC(x, y, 556);
					}
					num8 = NPC.NewNPC(x, y, 553);
				}
				break;
			case 4:
				if (Main.rand.Next(10) == 0 && NPC.CountNPCS(570) < num6)
				{
					num7 = NPC.NewNPC(x, y, 570);
				}
			...
			case 5:
				...
				{
					num7 = ((Main.rand.Next(2) != 0) ? NPC.NewNPC(x, y, 574) : NPC.NewNPC(x, y, 572));
				}
				else if (NPC.CountNPCS(553) + NPC.CountNPCS(556) < num)
				{
			...
			case 7:
			{
				...
				{
					num7 = ((Main.rand.Next(3) == 0) ? NPC.NewNPC(x, y, 574) : NPC.NewNPC(x, y, 572));
				}
		...
		private static short[] Difficulty_3_GetEnemiesForWave(int wave)
		{
			...
			case 6:
				LaneSpawnRate = 60;
				return new short[8] { 554, 557, 563, 560, 569, 571, 577, 565 };
			case 7:
				LaneSpawnRate = 90;
		...
		private static void Difficulty_3_SpawnMonsterFromGate(Vector2 gateBottom)
		{
			...
			case 1:
				...
				{
					num8 = NPC.NewNPC(x, y, 563);
				}
				else if (NPC.CountNPCS(554) < num)
				{
					if (Main.rand.Next(7) == 0)
					{
						num8 = NPC.NewNPC(x, y, 557);
					}
					num9 = NPC.NewNPC(x, y, 554);
				}
				break;
			case 2:
				if (Main.rand.Next(3) == 0 && NPC.CountNPCS(578) < num7)
				{
					num8 = NPC.NewNPC(x, y, 578);
				}
			...
			case 3:
				...
				{
					num8 = NPC.NewNPC(x, y, 554);
				}
				break;
			case 4:
				if (Main.rand.Next(24) == 0 && !NPC.AnyNPCs(565))
				{
					num8 = NPC.NewNPC(x, y, 565);
				}
				...
				{
					num8 = ((Main.rand.Next(3) == 0) ? NPC.NewNPC(x, y, 575) : NPC.NewNPC(x, y, 573));
				}
				else if (NPC.CountNPCS(554) < num)
				{
					num8 = NPC.NewNPC(x, y, 554);
			...
			case 5:
				...
				{
					num8 = ((Main.rand.Next(4) == 0) ? NPC.NewNPC(x, y, 575) : NPC.NewNPC(x, y, 573));
				}
				else if (NPC.CountNPCS(554) + NPC.CountNPCS(557) < num)
				{
					if (Main.rand.Next(3) == 0)
