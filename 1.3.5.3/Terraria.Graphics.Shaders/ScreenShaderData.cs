...
namespace Terraria.Graphics.Shaders
{
	public class ScreenShaderData : ShaderData
	{
		...
		public new virtual void Apply()
		{
			...
			for (int i = 0; i < _uImages.Length; i++)
			{
				if (_uImages[i] != null && _uImages[i].Value != null)
				{
					Main.graphics.GraphicsDevice.Textures[i + 1] = _uImages[i].Value;
					int width = _uImages[i].Value.Width;
					int height = _uImages[i].Value.Height;
		...
		public ScreenShaderData UseImage(Texture2D image, int index = 0, SamplerState samplerState = null)
		{
			...
			if (_uImages[index] == null)
			{
				_uImages[index] = new Ref<Texture2D>(image);
			}
			else
			{
				_uImages[index].Value = image;
			}
			return this;
		}
		...
		{
			_uImages[index] = TextureManager.AsyncLoad(path);
			_samplerStates[index] = samplerState;
			return this;
		}
