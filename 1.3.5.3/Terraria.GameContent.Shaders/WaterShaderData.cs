...
namespace Terraria.GameContent.Shaders
{
	public class WaterShaderData : ScreenShaderData
	{
			...
			: base(passName)
		{
			...
			_rippleShapeTexture = Main.instance.OurLoad<Texture2D>("Images/Misc/Ripples");
		...
		private void StepLiquids()
		{
			...
			if (_useViscosityFilter)
			{
				...
				tileBatch.Draw(_viscosityMaskChain[_activeViscosityMask], destination, value, new VertexColors(Color.White), Vector2.Zero, SpriteEffects.FlipHorizontally, (float)Math.PI / 2f);
				tileBatch.End();
		...
		private void DrawWaves()
		{
			...
			if (_useNPCWaves)
			{
				for (int i = 0; i < 200; i++)
				{
					...
					if (nPC.wet)
					{
						tileBatch.Draw(Main.magicPixel, new Vector4(vector4.X, vector4.Y, (float)nPC.width * 2f, (float)nPC.height * 2f) * 0.25f, null, new VertexColors(new Color(vector5.X * 0.5f + 0.5f, vector5.Y * 0.5f + 0.5f, 0.5f * num)), new Vector2((float)Main.magicPixel.Width / 2f, (float)Main.magicPixel.Height / 2f), SpriteEffects.None, nPC.rotation);
					}
					if (nPC.wetCount != 0)
					{
			...
			if (_usePlayerWaves)
			{
				for (int j = 0; j < 255; j++)
				{
					...
					if (player.wet)
					{
						tileBatch.Draw(Main.magicPixel, new Vector4(vector6.X - (float)player.width * 2f * 0.5f, vector6.Y - (float)player.height * 2f * 0.5f, (float)player.width * 2f, (float)player.height * 2f) * 0.25f, new VertexColors(new Color(velocity2.X * 0.5f + 0.5f, velocity2.Y * 0.5f + 0.5f, 0.5f * num3)));
					}
			...
			if (_useProjectileWaves)
			{
				for (int k = 0; k < 1000; k++)
				{
					...
					tileBatch.Draw(Main.magicPixel, new Vector4(vector7.X - num7 * 0.5f, vector7.Y - num8 * 0.5f, num7, num8) * 0.25f, new VertexColors(new Color(velocity3.X * 0.5f + 0.5f, velocity3.Y * 0.5f + 0.5f, num6 * 0.5f)));
				}
			}
			tileBatch.End();
			if (_useRippleWaves)
			{
				...
				for (int l = 0; l < _rippleQueueCount; l++)
				{
					...
					Texture2D rippleShapeTexture = _rippleShapeTexture;
					tileBatch.Draw(rippleShapeTexture, new Vector4(vector8.X, vector8.Y, size.X, size.Y) * 0.25f, sourceRectangle, new VertexColors(_rippleQueue[l].WaveData), new Vector2(sourceRectangle.Width / 2, sourceRectangle.Height / 2), SpriteEffects.None, _rippleQueue[l].Rotation);
				}
				tileBatch.End();
			}
			_rippleQueueCount = 0;
