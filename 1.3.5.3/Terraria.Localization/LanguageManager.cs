...
namespace Terraria.Localization
{
	public class LanguageManager
	{
		...
		{
			string cultureName = culture.CultureInfo.Name.Replace('-', '_');
			string prefix = "Terraria.Localization.Content." + cultureName;
			Assembly asm = typeof(Program).Assembly;
			string[] embeds = Array.FindAll(asm.GetManifestResourceNames(),
				(string element) => element.StartsWith(prefix) && element.EndsWith(".json"));
			...
			{
				return Directory.GetFiles(dir, prefix + "*.json");
			}
			return new string[0];
		}
		...
		private void LoadFilesForCulture(GameCulture culture)
		{
			...
			foreach (string text in languageFilesForCulture)
			{
				try
				{
					string text2 = ReadEmbeddedResource(text);
		...
		private static string ReadEmbeddedResource(string path)
		{
			...
			{
				filePath = Path.Combine(dir, Path.GetFileName(path));
			}
