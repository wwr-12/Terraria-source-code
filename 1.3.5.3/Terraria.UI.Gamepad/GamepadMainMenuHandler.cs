		...
		public static void Update()
		{
			...
			else
			{
				...
				for (int i = 0; i < MenuItemPositions.Count; i++)
				{
					...
					{
						Main.mouseX = (PlayerInput.MouseX = (int)MenuItemPositions[i].X);
						Main.mouseY = (PlayerInput.MouseY = (int)MenuItemPositions[i].Y);
						Main.menuFocus = -1;
					}
					UILinkPoint uILinkPoint = uILinkPage2.LinkMap[2000 + i];
					uILinkPoint.Position = MenuItemPositions[i];
					if (i == 0)
					{
						uILinkPoint.Up = -1;
					}
