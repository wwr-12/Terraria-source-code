	...
	public class WiresUI
	{
		...
		public class WiresRadial
		{
			...
			private void LineUpdate()
			{
				...
				if ((!player.showItemIcon && ((!PlayerInput.UsingGamepad && !Settings.DrawToolAllowActuators) || player.mouseInterface || player.lastMouseInterface)) || Main.ingameOptionsWindow || Main.InGameUI.IsVisible)
				{
					value = false;
					_lineOpacity = 0f;
					return;
			...
			private void DrawLine(SpriteBatch spriteBatch)
			{
				...
				for (int i = 0; i < 6; i++)
				{
					...
					Texture2D texture2D = Main.wireUITexture[(Settings.ToolMode.HasFlag(Settings.MultiToolMode.Cutter) ? 8 : 0) + (flag3 ? 1 : 0)];
					...
					case 3:
						...
						break;
					case 4:
						texture2D2 = Main.wireUITexture[Settings.ToolMode.HasFlag(Settings.MultiToolMode.Cutter) ? 7 : 6];
						break;
					case 5:
					...
					spriteBatch.Draw(texture2D, vector4, null, color * _lineOpacity, 0f, texture2D.Size() / 2f, _lineOpacity, SpriteEffects.None, 0f);
					spriteBatch.Draw(texture2D2, vector4, null, color2 * _lineOpacity, 0f, texture2D2.Size() / 2f, _lineOpacity, SpriteEffects.None, 0f);
				}
			...
			private void DrawFlower(SpriteBatch spriteBatch)
			{
				...
				for (int i = 0; i < 6; i++)
				{
					...
					spriteBatch.Draw(texture2D, vector4, null, color, 0f, texture2D.Size() / 2f, 1f, SpriteEffects.None, 0f);
					spriteBatch.Draw(texture2D2, vector4, null, color2, 0f, texture2D2.Size() / 2f, 1f, SpriteEffects.None, 0f);
				}
				if (Main.mouseLeft && Main.mouseLeftRelease && !flag)
				{
					active = false;
			...
			private void DrawCursorArea(SpriteBatch spriteBatch)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					...
					spriteBatch.Draw(builderAccTexture, vec, rectangle, color2 * num3, 0f, rectangle.Size() / 2f, scale, SpriteEffects.None, 0f);
					if (Settings.ToolMode.HasFlag(Settings.MultiToolMode.Cutter))
					{
						spriteBatch.Draw(builderAccTexture, vec, rectangle2, color * num3, 0f, rectangle2.Size() / 2f, scale, SpriteEffects.None, 0f);
					}
				}
			}
		}
