...
namespace Terraria.GameContent.Skies
{
	public class PartySky : CustomSky
	{
		...
		public override void OnLoad()
		{
			...
			{
				_textures[i] = Main.extraTexture[69 + i];
			}
		...
		public void ResetBalloon(int i)
		{
			...
			_balloons[i].Texture = _textures[_random.Next(2)];
			_balloons[i].Variant = _random.Next(3);
			if (_random.Next(30) == 0)
			{
				_balloons[i].Texture = _textures[2];
			}
		}
		...
		public override void Update(GameTime gameTime)
		{
			...
			for (int i = 0; i < _balloons.Length; i++)
			{
				...
				_balloons[i].Position.X += Main.windSpeed * (3f - _balloons[i].Speed);
