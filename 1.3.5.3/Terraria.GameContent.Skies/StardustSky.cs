	...
	public class StardustSky : CustomSky
	{
		...
		public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
		{
			...
			for (int j = num; j < num2; j++)
			{
				...
				{
					float value = (float)Math.Sin(_stars[j].AlphaFrequency * Main.GlobalTime + _stars[j].SinOffset) * _stars[j].AlphaAmplitude + _stars[j].AlphaAmplitude;
					...
					spriteBatch.Draw(texture2D, position, null, Color.White * num3 * value * 0.8f * (1f - num4) * _fadeOpacity, 0f, new Vector2(texture2D.Width >> 1, texture2D.Height >> 1), (vector4.X * 0.5f + 0.5f) * (value * 0.3f + 0.7f), SpriteEffects.None, 0f);
				}
			}
		}
