...
namespace Terraria.GameContent.Liquid
{
	public class LiquidRenderer
	{
		...
		private static readonly int[] WATERFALL_LENGTH = new int[3] { 10, 3, 2 };

		private static readonly float[] DEFAULT_OPACITY = new float[3] { 0.6f, 0.95f, 0.95f };

		private static readonly byte[] WAVE_MASK_STRENGTH = new byte[5] { 0, 0, 0, 255, 0 };
				...
				for (int i = drawArea.X; i < drawArea.X + drawArea.Width; i++)
				{
					for (int j = drawArea.Y; j < drawArea.Y + drawArea.Height; j++)
					{
						if (ptr2->IsVisible)
						{
							...
							num = Math.Min(1f, num);
							Lighting.GetColor4Slice_New(i, j, out var vertices);
							vertices.BottomLeftColor *= num;
							vertices.BottomRightColor *= num;
							vertices.TopLeftColor *= num;
							vertices.TopRightColor *= num;
							Main.tileBatch.Draw(_liquidTextures[num2], new Vector2(i << 4, j << 4) + drawOffset + liquidOffset, sourceRectangle, vertices, Vector2.Zero, 1f, SpriteEffects.None);
						}
						ptr2++;
					}
				}
		...
		public void Update(GameTime gameTime)
		{
			...
			{
				float value = Main.windSpeed * 80f;
				value = MathHelper.Clamp(value, -20f, 20f);
				value = ((!(value < 0f)) ? Math.Max(10f, value) : Math.Min(-10f, value));
				_frameState += value * (float)gameTime.ElapsedGameTime.TotalSeconds;
				if (_frameState < 0f)
				{
					_frameState += 16f;
				}
