...
namespace Terraria.GameContent.UI.States
{
	public class UIVirtualKeyboard : UIState
	{
		...
		public UIVirtualKeyboard(string labelText, string startingText, KeyboardSubmitEvent submitAction, Action cancelAction, int inputMode = 0, bool allowEmpty = false)
		{
			...
			_textureShift = TextureManager.Load("Images/UI/VK_Shift");
			_textureBackspace = TextureManager.Load("Images/UI/VK_Backspace");
			...
			_label = uIText;
			Append(uIElement);
			_textBox.SetTextMaxLength(_edittingSign ? 99999 : 20);
			Text = startingText;
			if (Text.Length == 0)
			{
		...
		private void TypeText(UIMouseEvent evt, UIElement listeningElement)
		{
			...
			_textBox.Write(((UITextPanel<object>)listeningElement).Text);
			ValidateText();
		...
		{
			if (Text.Trim().Length > 0 || _edittingSign || _edittingChest || _allowEmpty)
			{
				_canSubmit = true;
		...
		public static void Submit()
		{
			if (_currentInstance != null)
			{
				string text = _currentInstance.Text.Trim();
				if (text.Length > 0)
				{
					Main.PlaySound(10);
					_currentInstance._submitAction(text);
				}
			}
		}
