...
namespace Terraria.GameInput
{
	public class TriggersSet
	{
		...
		public TriggersSet Clone()
		{
			...
			foreach (string key in KeyStatus.Keys)
			{
				triggersSet.KeyStatus.Add(key, KeyStatus[key]);
			}
			triggersSet.UsedMovementKey = UsedMovementKey;
			triggersSet.HotbarScrollCD = HotbarScrollCD;
			triggersSet.HotbarHoldTime = HotbarHoldTime;
			return triggersSet;
		}
		...
		public void CopyInto(Player p)
		{
			...
			if (MouseRight && !p.mouseInterface && (!Main.blockMouse & !PlayerInput.LockTileUseButton) && !PlayerInput.InBuildingMode)
			{
