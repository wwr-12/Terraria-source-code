...
namespace Terraria.GameInput
{
	public class PlayerInput
	{
		...
		public static bool InvisibleGamepadInMenus
		{
			get
			{
				if ((!Main.gameMenu && !Main.ingameOptionsWindow && !Main.playerInventory && Main.player[Main.myPlayer].talkNPC == -1 && Main.player[Main.myPlayer].sign == -1) || _InBuildingMode || !Main.InvisibleCursorForGamepad)
				{
					if (CursorIsBusy)
					{
			...
			if (!CursorIsBusy)
			{
				...
				if (gamepadThumbstickLeft != Vector2.Zero && flag7)
				{
					...
					else if (!flag5 && Main.SmartCursorEnabled)
					{
						...
						Vector2 vector6 = gamepadThumbstickLeft * new Vector2(Player.tileRangeX * 16, Player.tileRangeY * 16) * m2;
						if (num3 != 0)
						{
							vector6 = gamepadThumbstickLeft * new Vector2((Player.tileRangeX + num3) * 16, (Player.tileRangeY + num3) * 16) * m2;
						}
						if (flag4)
						{
				...
				if (CurrentInputMode == InputMode.XBoxGamepad)
				{
					...
					if (!Main.gameMenu && !flag3)
					{
						...
						else
						{
							...
							num11 = (int)Utils.Clamp(num11, (float)(-(Player.tileRangeX + num3) * 16) * m3, (float)((Player.tileRangeX + num3) * 16) * m3);
							num12 = (int)Utils.Clamp(num12, (float)(-(Player.tileRangeY + num3) * 16) * m3, (float)((Player.tileRangeY + num3) * 16) * m3);
						}
						if (flag6 && (!flag || flag4))
						{
							float num16 = 0.81f;
							...
							num11 = (int)((float)num11 * num16);
							num12 = (int)((float)num12 * num16);
						}
					}
					else
					{
		...
		private static void MouseInput()
		{
			...
			int num = MouseInfo.X - MouseInfoOld.X;
			int num2 = MouseInfo.Y - MouseInfoOld.Y;
			if (num != 0 || num2 != 0 || MouseInfo.ScrollWheelValue != MouseInfoOld.ScrollWheelValue)
			{
				MouseX = MouseInfo.X;
				MouseY = MouseInfo.Y;
				flag = true;
		...
		private static void KeyboardInput()
		{
			...
			bool flag2 = false;
			Keys[] pressedKeys = Main.keyState.GetPressedKeys();
			if (InvalidateKeyboardSwap() && MouseKeys.Count == 0)
			{
				return;
			}
			for (int i = 0; i < pressedKeys.Length; i++)
			...
			foreach (InputMode value in Enum.GetValues(typeof(InputMode)))
			{
				...
				foreach (string item in list)
				{
					if (CurrentProfile.InputModes[value].KeyStatus[item].Count < 1)
					{
						...
						CurrentProfile.InputModes[value].KeyStatus[item].AddRange(OriginalProfiles[key].InputModes[value].KeyStatus[item]);
					}
				}
			}
		}
		...
		private static void HandleDpadSnap()
		{
			...
			if (zero != Vector2.Zero)
			{
				...
				Point point = (Main.MouseScreen + Main.screenPosition + zero * new Vector2(16f)).ToTileCoordinates();
				MouseX = point.X * 16 + 8 - (int)Main.screenPosition.X;
				MouseY = point.Y * 16 + 8 - (int)Main.screenPosition.Y;
			}
		}
		...
		public static string ComposeInstructionsForGamepad()
		{
			...
			if (Main.mapFullscreen && !Main.gameMenu)
			{
				...
				text += BuildCommand(Lang.inter[118].Value, false, ProfileGamepadUI.KeyStatus["HotbarMinus"]);
				text += BuildCommand(Lang.inter[119].Value, false, ProfileGamepadUI.KeyStatus["HotbarPlus"]);
			...
			{
				if (!GrappleAndInteractAreShared || (!WiresUI.Settings.DrawToolModeUI && (!Main.SmartInteractShowingGenuine || (Main.SmartInteractNPC == -1 && (Main.SmartInteractX == -1 || Main.SmartInteractY == -1)))))
				{
		...
		public static string BuildCommand(string CommandText, bool Last, params List<string>[] Bindings)
		{
			...
			text += GenInput(Bindings[0]);
			for (int i = 1; i < Bindings.Length; i++)
			{
				string text2 = GenInput(Bindings[i]);
				if (text2.Length > 0)
				{
					text = text + "/" + text2;
				}
