...
namespace Terraria.Map
{
	public static class MapHelper
	{
		...
		public static void SaveMap()
		{
			...
			if ((isCloudSave && SocialAPI.Cloud == null) || !Main.mapEnabled || saveLock)
			{
				return;
			}
			...
			lock (padlock)
			{
				try
				{
					...
					text = ((!Main.ActiveWorldFileData.UseGuidAsMapName) ? (text + Main.worldID + ".map") : (text + Main.ActiveWorldFileData.UniqueId.ToString() + ".map"));
					new Stopwatch().Start();
					bool flag = false;
					if (!Main.gameMenu)
					{
