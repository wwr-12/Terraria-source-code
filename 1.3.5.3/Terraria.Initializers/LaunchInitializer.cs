...
namespace Terraria.Initializers
{
	public static class LaunchInitializer
	{
		...
		private static void LoadServerParameters(Main game)
		{
			...
			string s2;
			if ((s2 = TryParameter("-maxplayers", "-players")) != null && int.TryParse(s2, out var result2))
			{
			...
			string s3;
			if ((s3 = TryParameter("-lang")) != null && int.TryParse(s3, out var result3))
			{
				LanguageManager.Instance.SetLanguage(result3);
			}
