	...
	public class MahoganyTreeBiome : MicroBiome
	{
		public override bool Place(Point origin, StructureMap structures)
		{
			...
			for (int j = 0; j < 2; j++)
			{
				...
				double num13 = GenBase._random.NextDouble() * 0.7853981852531433 - 0.7853981852531433 - 0.20000000298023224;
				if (num10 == 0)
				{
			...
			WorldUtils.Gen(new Point(result.X + 6 + num14, result.Y - num4), new ShapeBranch(-0.6853981852531433, GenBase._random.Next(16, 22)).OutputEndpoints(list), Actions.Chain(new Actions.SetTile(383), new Actions.SetFrames(frameNeighbors: true)));
			...
			WorldGen.AddBuriedChest(result.X + 3, result.Y - 1, (GenBase._random.Next(4) != 0) ? WorldGen.GetNextJungleChestItem() : 0, notNearOtherChests: false, 10);
