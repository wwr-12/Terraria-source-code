...
namespace Terraria
{
	public class Lang
	{
		...
		public static string GetDryadWorldStatusDialog()
		{
			...
			string arg = (((double)tGood * 1.2 >= (double)(tEvil + tBlood) && (double)tGood * 0.8 <= (double)(tEvil + tBlood)) ? Language.GetTextValue("DryadSpecialText.WorldDescriptionBalanced") : ((tGood >= tEvil + tBlood) ? Language.GetTextValue("DryadSpecialText.WorldDescriptionFairyTale") : ((tEvil + tBlood > tGood + 20) ? Language.GetTextValue("DryadSpecialText.WorldDescriptionGrim") : ((tEvil + tBlood <= 10) ? Language.GetTextValue("DryadSpecialText.WorldDescriptionClose") : Language.GetTextValue("DryadSpecialText.WorldDescriptionWork")))));
			return $"{text} {arg}";
		}
		...
		public static string BartenderHelpText(NPC npc)
		{
			...
			if (player.bartenderQuestLog == 0)
			{
				...
				item = player.GetItem(player.whoAmI, item, longText: true);
				if (item.stack > 0)
				{
		...
		private static void FillNameCacheArray<IdClass, IdType>(string category, LocalizedText[] nameCache, bool leaveMissingEntriesBlank = false) where IdType : IConvertible
		{
				...
				select f).ToList().ForEach(delegate(FieldInfo field)
			{
				...
				else if (num == 0L && field.Name == "None")
				{
		...
		public static void BuildMapAtlas()
		{
			...
			_mapLegendCache[MapHelper.TileToLookup(5, 0)] = Language.GetText("MapObject.Tree");
			...
			for (int j = 0; j < 9; j++)
			{
				_mapLegendCache[MapHelper.TileToLookup(28, j)] = Language.GetText("MapObject.Pot");
			}
			...
			_mapLegendCache[MapHelper.TileToLookup(413, 0)] = Language.GetText("MapObject.OrangeSquirrelCage");
