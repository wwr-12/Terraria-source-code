...
namespace Terraria
{
	public static class Utils
	{
		...
		public static bool PlotTileLine(Vector2 start, Vector2 end, float width, PerLinePoint plot)
		{
			...
			Vector2 vector2 = vector / vector.Length();
			Vector2 vector3 = new Vector2(0f - vector2.Y, vector2.X) * num;
			Point point = (start - vector3).ToTileCoordinates();
			Point point2 = (start + vector3).ToTileCoordinates();
		...
		public static bool PlotTileTale(Vector2 start, Vector2 end, float width, PerLinePoint plot)
		{
			...
			Vector2 vector = end - start;
			Vector2 vector2 = vector / vector.Length();
			Vector2 perpOffset = new Vector2(0f - vector2.Y, vector2.X);
			...
			{
				float num = 1f - (float)curLength / (float)length;
				curLength++;
		...
		public static Vector2 DrawBorderStringBig(SpriteBatch spriteBatch, string text, Vector2 pos, Color color, float scale = 1f, float anchorx = 0f, float anchory = 0f, int maxCharactersDisplayed = -1)
		{
			...
			for (int i = -1; i < 2; i++)
			{
				for (int j = -1; j < 2; j++)
				{
					DynamicSpriteFontExtensionMethods.DrawString(spriteBatch, fontDeathText, text, pos + new Vector2(i, j), Color.Black, 0f, new Vector2(anchorx, anchory) * fontDeathText.MeasureString(text), scale, SpriteEffects.None, 0f);
				}
			}
			DynamicSpriteFontExtensionMethods.DrawString(spriteBatch, fontDeathText, text, pos, color, 0f, new Vector2(anchorx, anchory) * fontDeathText.MeasureString(text), scale, SpriteEffects.None, 0f);
		...
		{
			DrawPanel(Main.settingsPanelTexture, 2, 0, spriteBatch, position, width, color);
		}
		...
		public static void DrawLine(SpriteBatch spriteBatch, Vector2 start, Vector2 end, Color color)
		{
			...
			{
				float num3 = num2 / num;
				spriteBatch.Draw(Main.blackTileTexture, vector2 - screenPosition, null, new Color(new Vector4(num3, num3, num3, 1f) * color.ToVector4()), rotation, Vector2.Zero, 0.25f, SpriteEffects.None, 0f);
		...
		public static void DrawLine(SpriteBatch spriteBatch, Vector2 start, Vector2 end, Color colorStart, Color colorEnd, float width)
		{
			...
			for (float num2 = 0f; num2 <= num; num2 += width)
			{
				...
				spriteBatch.Draw(Main.blackTileTexture, vector2 - screenPosition, null, Color.Lerp(colorStart, colorEnd, amount), rotation, Vector2.Zero, scale, SpriteEffects.None, 0f);
				vector2 = start + num2 * vector;
			}
		}
		...
		public static void DrawCursorSingle(SpriteBatch sb, Color color, float rot = float.NaN, float scale = 1f, Vector2 manualPosition = default(Vector2), int cursorSlot = 0, int specialMode = 0)
		{
			...
			if (flag2)
			{
				sb.Draw(Main.cursorTextures[cursorSlot], vector + vector2 + Vector2.One, null, color.MultiplyRGB(new Color(0.2f, 0.2f, 0.2f, 0.5f)), rot, origin, scale * 1.1f, SpriteEffects.None, 0f);
			}
			if (flag3)
			{
				sb.Draw(Main.cursorTextures[cursorSlot], vector + vector2, null, color, rot, origin, scale, SpriteEffects.None, 0f);
			}
		}
	}
}
