		...
		public static void LavaCheck(int x, int y)
		{
			...
			if ((tile.liquid > 0 && !tile.lava()) || (tile2.liquid > 0 && !tile2.lava()) || (tile3.liquid > 0 && !tile3.lava()))
			{
				...
				if (!tile5.active())
				{
					...
					if (num2 == 56)
					{
						Main.PlaySound(SoundID.LiquidsWaterLava, new Vector2(x * 16 + 8, y * 16 + 8));
					}
					else
					{
						Main.PlaySound(SoundID.LiquidsHoneyLava, new Vector2(x * 16 + 8, y * 16 + 8));
					}
					WorldGen.PlaceTile(x, y, num2, mute: true, forced: true);
			...
			else
			{
				...
				bool flag = false;
				if (tile5.active() && TileID.Sets.ForceObsidianKill[tile5.type] && !TileID.Sets.ForceObsidianKill[tile4.type])
				{
					flag = true;
				}
				...
				WorldGen.PlaceTile(x, y + 1, num3, mute: true, forced: true);
		...
		public static void HoneyCheck(int x, int y)
		{
			...
			if ((tile.liquid > 0 && tile.liquidType() == 0) || (tile2.liquid > 0 && tile2.liquidType() == 0) || (tile3.liquid > 0 && tile3.liquidType() == 0))
			{
				...
				if (!tile5.active())
				{
					tile5.liquid = 0;
					tile5.liquidType(0);
					WorldGen.PlaceTile(x, y, 229, mute: true, forced: true);
					if (flag)
					{
						Main.PlaySound(SoundID.LiquidsHoneyLava, new Vector2(x * 16 + 8, y * 16 + 8));
					}
					...
					{
						Main.PlaySound(SoundID.LiquidsHoneyWater, new Vector2(x * 16 + 8, y * 16 + 8));
					}
					WorldGen.SquareTileFrame(x, y);
					if (Main.netMode == 2)
					{
			...
			else
			{
				...
				WorldGen.PlaceTile(x, y + 1, 229, mute: true, forced: true);
				WorldGen.SquareTileFrame(x, y + 1);
				if (Main.netMode == 2)
				{
