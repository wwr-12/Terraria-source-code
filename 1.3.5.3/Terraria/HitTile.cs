		...
		public void DrawFreshAnimations(SpriteBatch spriteBatch)
		{
			...
			for (int j = 0; j < data.Length; j++)
			{
				...
				if (!((float)animationTimeElapsed >= 10f))
				{
					...
					spriteBatch.Draw(texture, position, new Rectangle(tile.frameX, tile.frameY, 16, 16), color, rotation, vector2, scale, SpriteEffects.None, 0f);
					color.A = 180;
					spriteBatch.Draw(Main.tileCrackTexture, position, value, color, rotation, vector2, scale, SpriteEffects.None, 0f);
				}
			}
		}
	}
