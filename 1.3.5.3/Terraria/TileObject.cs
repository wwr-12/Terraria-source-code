...
namespace Terraria
{
	public struct TileObject
	{
			...
			while (num7 < num3)
			{
					...
					for (int k = 0; k < anchorBottom.tileCount; k++)
					{
						...
						if (tileSafely.nactive())
						{
							...
							if (!flag5 && ((anchorBottom.type & AnchorType.SolidWithTop) == AnchorType.SolidWithTop || (anchorBottom.type & AnchorType.Table) == AnchorType.Table))
							{
								...
								{
									int num27 = tileSafely.frameX / TileObjectData.PlatformFrameWidth();
				...
				if (anchorBottom.tileCount != 0)
				{
					...
					for (int l = 0; l < anchorBottom.tileCount; l++)
					{
						...
						if (!flag6)
						{
							if (onlyCheck)
							{
								objectPreview[num30 + num10, num29 + num11] = 2;
							}
							continue;
						}
						if (onlyCheck)
						{
							objectPreview[num30 + num10, num29 + num11] = 1;
						}
						num24 += 1f;
					}
				}
				...
				if (anchorBottom.tileCount != 0)
				{
					...
					for (int m = 0; m < anchorBottom.tileCount; m++)
					{
							...
							if (!flag7 && (anchorBottom.type & AnchorType.Tree) == AnchorType.Tree && tileSafely.type == 5)
							{
								...
								if (m == 0)
								{
									...
									if (tileSafely2.nactive() && tileSafely2.type == 5)
									{
										...
										if (onlyCheck)
										{
											objectPreview[width + num10, num31 + num11 - 1] = 1;
										}
									}
									else if (onlyCheck)
									{
										objectPreview[width + num10, num31 + num11 - 1] = 2;
									}
								}
						...
						if (!flag7)
						{
							if (onlyCheck)
							{
								objectPreview[width + num10, num31 + num11] = 2;
							}
							continue;
						}
						if (onlyCheck)
						{
							objectPreview[width + num10, num31 + num11] = 1;
						}
						num24 += 1f;
					}
				}
				...
				if (anchorBottom.tileCount != 0)
				{
					...
					for (int n = 0; n < anchorBottom.tileCount; n++)
					{
						...
						Tile tileSafely = Framing.GetTileSafely(num8 + num32, num9 + num33);
						bool flag8 = false;
						if (tileSafely.nactive())
						{
							if (Main.tileSolid[tileSafely.type] && !Main.tileSolidTop[tileSafely.type] && !Main.tileNoAttach[tileSafely.type] && (tileData2.FlattenAnchors || tileSafely.blockType() == 0))
							...
							if (!flag8 && (anchorBottom.type & AnchorType.Tree) == AnchorType.Tree && tileSafely.type == 5)
							{
								...
								{
									num25 += 1f;
									Tile tileSafely4 = Framing.GetTileSafely(num8 + num32, num9 + num33 - 1);
									if (tileSafely4.nactive() && tileSafely4.type == 5)
									{
										num24 += 1f;
										if (onlyCheck)
										{
											objectPreview[num32 + num10, num33 + num11 - 1] = 1;
										}
									}
									else if (onlyCheck)
									{
										objectPreview[num32 + num10, num33 + num11 - 1] = 2;
									}
								}
								if (n == anchorBottom.tileCount - 1)
								{
									num25 += 1f;
									Tile tileSafely5 = Framing.GetTileSafely(num8 + num32, num9 + num33 + 1);
						...
						if (!flag8)
						{
							if (onlyCheck)
							{
								objectPreview[num32 + num10, num33 + num11] = 2;
							}
							continue;
						}
						if (onlyCheck)
						{
							objectPreview[num32 + num10, num33 + num11] = 1;
						}
						num24 += 1f;
					}
				}
		...
		public static void DrawPreview(SpriteBatch sb, TileObjectPreviewData op, Vector2 position)
		{
			...
			if (tileData.StyleWrapLimit > 0)
			{
				num4 = num3 / tileData.StyleWrapLimit * tileData.StyleLineSkip;
