...
namespace Terraria.GameContent.UI.Elements
{
	public class UIKeybindingToggleListItem : UIElement
	{
		...
		protected override void DrawSelf(SpriteBatch spriteBatch)
		{
			...
			position.X += 8f;
			position.Y += 2f + num;
			ChatManager.DrawColorCodedStringWithShadow(spriteBatch, Main.fontItemStack, _TextDisplayFunction(), position, value, 0f, Vector2.Zero, baseScale, num2);
			position.X -= 17f;
			...
			spriteBatch.Draw(position: new Vector2(dimensions.X + dimensions.Width - vector2.X - 10f, dimensions.Y + 2f + num), texture: _toggleTexture, sourceRectangle: value2, color: Color.White, rotation: 0f, origin: Vector2.Zero, scale: Vector2.One, effects: SpriteEffects.None, layerDepth: 0f);
		}
	}
}
