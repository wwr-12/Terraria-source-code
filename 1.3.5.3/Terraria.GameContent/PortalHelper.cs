...
namespace Terraria.GameContent
{
	public class PortalHelper
	{
		...
		public static int TryPlacingPortal(Projectile theBolt, Vector2 velocity, Vector2 theCrashVelocity)
		{
			...
			new Vector2(position.X * 16 + 8, position.Y * 16 + 8);
			...
			for (int i = 0; i < (flag ? 2 : EDGES.Length); i++)
			{
				...
				{
					return AddPortal(new Vector2(bestPosition.X * 16 + 8, bestPosition.Y * 16 + 8) - EDGES[i] * (flag ? 0f : 8f), (float)Math.Atan2(EDGES[i].Y, EDGES[i].X) + (float)Math.PI / 2f, (int)theBolt.ai[0], theBolt.direction);
				}
			}
			if (num != 0)
			{
				...
				{
					return AddPortal(new Vector2(bestPosition2.X * 16 + 8, bestPosition2.Y * 16 + 8), (float)Math.Atan2(value.Y, value.X) - (float)Math.PI / 2f, (int)theBolt.ai[0], theBolt.direction);
				}
			}
			return -1;
		}
		...
		private static void RemoveIntersectingPortals(Vector2 position, float angle)
		{
			...
			for (int i = 0; i < 1000; i++)
			{
				...
				if (Collision.CheckLinevLine(start, end, start2, end2).Length != 0)
				{
					...
					{
						NetMessage.SendData(95, -1, -1, null, i);
					}
					projectile.Kill();
					if (Main.netMode == 2)
					{
						NetMessage.SendData(29, -1, -1, null, projectile.whoAmI, projectile.owner);
					}
				}
			}
		}
