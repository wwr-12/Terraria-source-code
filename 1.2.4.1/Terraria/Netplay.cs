...
namespace Terraria
{
	public class Netplay
	{
		...
		{
			portForwardIP = LocalIPAddress();
			portForwardPort = serverPort;
		}
		...
		public static void AddBan(int plr)
		{
			...
			using (StreamWriter streamWriter = new StreamWriter(banFile, true))
			{
		...
		public static void ClientLoop(object threadContext)
		{
			...
			while (!disconnect)
			{
				if (clientSock.tcpClient.Connected)
				{
					...
					if (clientSock.state == 5 && Main.loadMapLock)
					{
						float num2 = (float)Main.loadMapLastX / (float)Main.maxTilesX;
						Main.statusText = Lang.gen[68] + " " + (int)(num2 * 100f + 1f) + "%";
					}
					else if (clientSock.state == 5 && WorldGen.worldCleared)
					{
						clientSock.state = 6;
